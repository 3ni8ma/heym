...
```json
{
  ...
  "data": {
    "label": "createTask",
    "credentialId": "<notion-credential-uuid>",
    "notionOperation": "createPage",
    "notionDataSourceId": "<data-source-id>",
    "notionProperties": "{\"Name\":{\"title\":[{\"text\":{\"content\":\"$input.title\"}}]}}",
    "notionChildren": "[]"
  }
}
```
...
- **Data fields**:
  ...
  - `playwrightStealth`: Boolean - opt-in Steps-mode setting that reduces common Playwright/Chromium automation signals (default: false). UI label: "Reduce automation flags". When true, generated Chromium launch drops `--enable-automation`, uses a standard Chrome user agent matching the host OS, and restores `navigator.webdriver` / `window.chrome` / plugins so sites are less likely to treat the session as a bot. Linux/Docker reports a typical Mali or Mesa GPU string instead of SwiftShader; do not invent a Mac GPU. Does not bypass site security and does not apply to Run Code mode. Enable only for authorized automation when HeadlessChrome is blocked.
...
| `getAll` | none | List owned and team-shared readable Drive files as metadata objects |
...
- `$nodeLabel.count` - Number of returned files (getAll only)
- `$nodeLabel.status` - `"deleted"` (delete), `"updated"` (set* operations), or `"success"` (shareWithMyTeams/unshareWithMyTeams)
...
- `$nodeLabel.shared_team_count` - Number of teams the file is shared with (shareWithMyTeams/unshareWithMyTeams)
