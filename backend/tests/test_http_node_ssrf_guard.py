...
class Ipv6TransitionAddressTests(unittest.TestCase):
    ...
    def test_sixtofour_and_teredo_stay_blocked_even_when_public(self) -> None:
        ...
        # Refused by the guard itself, not by is_global: the stdlib only started
        # classifying these as private in 3.11.10 (CVE-2024-4032), and the project
        # supports >=3.11, so the check must not depend on the interpreter.
        for label, literal in (
