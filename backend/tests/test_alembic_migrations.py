...
class AlembicMigrationGraphTest(unittest.TestCase):
    ...
    def test_alerts_revision_follows_heym_events_revision(self) -> None:
        alerts_revision = self.script.get_revision("108_add_alerts")

        self.assertIsNotNone(alerts_revision)
        self.assertEqual(alerts_revision.down_revision, "107_add_heym_events")
    ...
    def test_mcp_chat_tool_revision_follows_rag_credential_revision(self) -> None:
        ...
        self.assertIsNotNone(chat_tool_revision)
        self.assertEqual(chat_tool_revision.down_revision, "105_add_rag_credential_type")
    ...
    def test_live_execution_snapshot_revision_follows_opencode_revision(self) -> None:
        ...
        self.assertEqual(
            ...
            "100_add_opencode_credential_type",
        )

    def test_live_and_user_ai_heads_are_merged(self) -> None:
        merge_revision = self.script.get_revision("102_merge_user_ai_live_heads")

        self.assertIsNotNone(merge_revision)
        self.assertEqual(
            set(merge_revision.down_revision),
            {"101_add_user_ai_defaults", "101_add_live_execution_snapshots"},
        )
