...
function handleNodeDoubleClick(event: {
  ...
  if (nodeType === "llm") {
    ...
    const sentryOperation = event.node.data?.sentryOperation;
    if (sentryOperation === "getIssue" || sentryOperation === "updateIssue") {
...
function getDefaultNodeData(type: NodeType): WorkflowNode["data"] {
  const defaults: Record<NodeType, WorkflowNode["data"]> = {
    ...
    converter: { label: "converter", conversion: "csvToJson", source: "$input.text", delimiter: ",", hasHeader: true, includeHeader: true, converterColumns: "" },
    jsonOutputMapper: {
      label: "jsonResponse",
