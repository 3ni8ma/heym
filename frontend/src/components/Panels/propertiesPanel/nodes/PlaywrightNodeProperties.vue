...
const PLAYWRIGHT_CODE_PLACEHOLDER = `from playwright.sync_api import sync_playwright
import json
...
<template>
  <template v-if="selectedNode">
    <div class="space-y-4">
      <div
        class="space-y-2"
        data-testid="playwright-mode-field"
      >
        <Label>Mode</Label>
        <Select
          :model-value="playwrightMode"
          :options="playwrightModeOptions"
          @update:model-value="setPlaywrightMode(($event as 'steps' | 'code'))"
        />
        <p class="text-xs text-muted-foreground">
          Steps builds browser actions visually. Run Code executes custom Playwright Python in the
          hardened sandbox (requires
          <code class="font-mono">HEYM_PLAYWRIGHT_CUSTOM_CODE_ENABLED=true</code>).
        </p>
      </div>

      <template v-if="playwrightMode === 'code'">
        <div
          class="space-y-2"
          data-testid="playwright-code-field"
        >
          <Label>Playwright code</Label>
          <Textarea
            :model-value="selectedNode.data.playwrightCode || ''"
            :placeholder="PLAYWRIGHT_CODE_PLACEHOLDER"
            :rows="16"
            wrap="off"
            class="font-mono text-xs leading-relaxed"
            @update:model-value="updateNodeData('playwrightCode', $event)"
          />
          <p class="text-xs text-muted-foreground">
            Available in the wrapper:
            <code class="font-mono">inputs</code>,
            <code class="font-mono">headless</code>,
            <code class="font-mono">timeout_ms</code>,
            <code class="font-mono">capture_network</code>.
            Use
            <code class="font-mono">$nodeLabel.field</code>
            expressions in the code; print a JSON object with
            <code class="font-mono">status</code>
            and
            <code class="font-mono">results</code>.
            In the sandbox, launch Chromium with
            <code class="font-mono">args=["--no-sandbox"]</code>.
          </p>
        </div>
      </template>
